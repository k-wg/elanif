...
def display_dino(message, dino_type="default", title=""):
    """Display ASCII art with message"""
    try:
...
def display_strategy_activation(strategy_variant, daily_diff):
    """Display strategy activation with art"""
...
def process_entry_signal(row, strategy_type):
    ...
    if trade_state.mode == TradingMode.BUY:
        ...
        with state_lock:
            ...
            # Capture fibo_0 for sell mode option 2
...
def process_single_row(row):
    ...
    # Sell mode option 2: Check MA_200 <= captured_fibo_0_sell
    if (
        trade_state.mode == TradingMode.SELL
        ...
        if pd.notna(ma_200) and ma_200 <= trade_state.captured_fibo_0_sell:
            print_signal("SELL MODE OPTION 2 EXIT", "MA_200 <= Captured Fibo_0")
